...
import { SpinningBox } from '@/components/3rd/performance/SpinningBox'
import { Fonts, Models } from '../types'
//THREE.ColorManagement.legacyMode = false

const context = createContext({})
export function ComputerInstances({ children, ...props }) {
  const { nodes } = useGLTF(Models.Computers)
...
export function Computers(props) {
  const { nodes: n, materials: m } : {
    nodes: any,
    materials: any
  } = useGLTF(Models.Computers)
  ...
  return (
    <group {...props} dispose={null}>
      ...
      <ScreenText frame="Object_209" panel="Object_210" y={5} position={[-1.43, 2.5, -1.8]} rotation={[0, 1, 0]} invert={undefined} />
      ...
      <ScreenText frame="Object_221" panel="Object_222" y={5} position={[-3.42, 3.06, 1.3]} rotation={[0, 1.22, 0]} scale={0.9} invert={undefined} />
      <ScreenText invert frame="Object_224" panel="Object_225" position={[-3.9, 4.29, -2.64]} rotation={[0, 0.54, 0]} />
      <ScreenText frame="Object_227" panel="Object_228" position={[0.96, 4.28, -4.2]} rotation={[0, -0.65, 0]} invert={undefined} />
      <ScreenText frame="Object_230" panel="Object_231" position={[4.68, 4.29, -1.56]} rotation={[0, -Math.PI / 3, 0]} invert={undefined} />
...
function ScreenText({ invert, x = 0, y = 1.2, ...props }) {
  ...
  return (
    <Screen frame={undefined} panel={undefined} {...props}> {/** children={undefined} */}
      <PerspectiveCamera makeDefault manual aspect={1 / 1} position={[0, 0, 15]} />
      <color attach="background" args={[invert ? 'black' : '#D4664A']} />
      <ambientLight intensity={0.5} />
      <directionalLight position={[10, 10, 5]} />
      <Text font={Fonts.InterMenium} position={[x, y, 0]} ref={textRef} fontSize={4} letterSpacing={-0.1} color={!invert ? 'black' : '#35c19f'}>
        Alterworld
      </Text>
    </Screen>
  )
